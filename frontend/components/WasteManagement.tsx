...
import { isCameraOffline, getCameraStatusMessage, deduplicateBins } from '../utils/binHelpers'; // Import helpers
...
const WasteManagement: React.FC<WasteManagementProps> = ({ 
  ...
  useEffect(() => {
      ...
      // Render Bins (deduplicated to prevent showing 65 bins when backend has 18)
      const uniqueBins = deduplicateBins(bins);
      uniqueBins.forEach(bin => {
          // Check if camera is offline (no data in last 10 hours)
          const cameraOffline = isCameraOffline(bin);
          // Color: red if camera offline, red if full (>80%), green otherwise
          const color = cameraOffline ? '#dc2626' : (bin.fillLevel > 80 ? '#ef4444' : '#10b981');
          const marker = window.L.marker([bin.location.lat, bin.location.lng], {
              icon: window.L.divIcon({
                  className: 'bg-transparent',
                  html: `<div style="background-color: ${color};" class="w-6 h-6 rounded-full border-2 border-white shadow-xl flex items-center justify-center text-white relative ${cameraOffline ? 'animate-pulse' : ''}">
                            ...
                            ${cameraOffline ? '<div class="absolute -bottom-1 -right-1 w-2 h-2 bg-red-600 rounded-full border border-white animate-ping"></div>' : ''}
                         </div>`
              })
  ...
  const handleSaveBin = () => {
      ...
      if (editingBin && onUpdateBin) {
          ...
          const bin: WasteBin = {
              id: `NEW-${Date.now()}`, // Temporary ID - NOT UUID format so backend creates new bin via POST
              location: { lat: latNum, lng: lngNum },
              address: newBin.address,
              tozaHudud: newBin.tozaHudud,
              cameraUrl: newBin.cameraUrl,
       ...
       <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-80 border-r border-slate-100 bg-white flex flex-col overflow-hidden">
             ...
             <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-2">
                {activeTab === 'BINS' ? (
                    // Filter unique bins by ID to prevent duplicates (using helper)
                    deduplicateBins(bins).map(bin => {
                        const cameraOffline = isCameraOffline(bin);
                        return (
                        <div 
                            key={bin.id} 
                            ...
                            className={`p-4 rounded-2xl border transition-all cursor-pointer relative overflow-hidden group ${selectedEntity?.data.id === bin.id ? 'bg-indigo-50 border-indigo-200 shadow-sm' : cameraOffline ? 'bg-red-50 border-red-200' : 'bg-white border-slate-100 hover:bg-slate-50'}`}
                        >
                            ...
                            <div className="flex items-center justify-between text-[9px] font-bold text-slate-400">
                                ...
                                <div className="flex gap-1">
                                    ...
                                    {cameraOffline && <span className="bg-red-100 text-red-600 px-1.5 py-0.5 rounded flex items-center gap-1"><AlertCircle size={8}/> OFFLINE</span>}
                                       ...
                                       <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100">
                                           ...
                                           <div className="space-y-3">
                                               ...
                                               <div className="flex justify-between items-center p-3 bg-slate-50 rounded-xl">
                                                   ...
                                                   <span className="text-xs font-mono font-bold text-slate-800">{selectedEntity.data.lastAnalysis || 'Ma\'lumot yo\'q'}</span>
                                               </div>
                                               <div className="flex justify-between items-center p-3 bg-slate-50 rounded-xl">
                                                   <span className="text-xs font-bold text-slate-600">Kamera holati</span>
                                                   {isCameraOffline(selectedEntity.data) ? (
                                                       <span className="text-[10px] font-bold text-red-600 bg-red-100 px-2 py-1 rounded animate-pulse flex items-center gap-1">
                                                           <AlertCircle size={10}/> {getCameraStatusMessage(selectedEntity.data)}
                                                       </span>
                                                   ) : (
                                                       <span className="text-[10px] font-bold text-emerald-600 bg-emerald-100 px-2 py-1 rounded">ONLINE • Ishlamoqda</span>
                                                   )}
                                               </div>
                                               <div className="flex justify-between items-center p-3 bg-slate-50 rounded-xl">
