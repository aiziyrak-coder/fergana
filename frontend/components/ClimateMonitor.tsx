                        ...
                        return (
                            ...
                            <div className="space-y-2 mb-3">
                              ...
                              {(displayTemperature !== undefined || boilerIoTDevice) && (() => {
                                  const tStatus = displayTemperature !== undefined ? getTemperatureStatusColor(displayTemperature, config.tempMin, config.tempMax) : 'OK';
                                  const tDirection = displayTemperature !== undefined ? getTemperatureDirection(displayTemperature, config.tempMin, config.tempMax) : 'NORMAL';
                                  let tempCardClass = "bg-blue-50 border-blue-100";
                                  let tempIconBg = "bg-blue-100";
                                  let tempIconColor = "text-blue-600";
                                  let tempValueClass = "bg-blue-100 text-blue-800";
                                  let tempLabelColor = "text-blue-400";
                                  
                                  if (tStatus === 'CRITICAL') {
                                    tempCardClass = "bg-red-500 text-white border-red-600 shadow-md shadow-red-500/20";
                                    tempIconBg = "bg-white/20";
                                    tempIconColor = "text-white animate-pulse";
                                    tempValueClass = "bg-white/20 text-white";
                                    tempLabelColor = "text-white/70";
                                  } else if (tStatus === 'WARNING') {
                                    tempCardClass = "bg-amber-100 border-amber-200";
                                    tempIconBg = "bg-amber-200";
                                    tempIconColor = "text-amber-600";
                                    tempValueClass = "bg-amber-200 text-amber-800";
                                    tempLabelColor = "text-amber-500";
                                  }
                                  
                                  return (
                                  <div className={`rounded-[14px] p-3 border flex items-center justify-between transition-colors ${tempCardClass}`}>
                                      <div className="flex items-center gap-3">
                                          <div className={`p-2 rounded-full ${tempIconBg}`}>
                                              <Thermometer size={18} className={tempIconColor} />
                                          </div>
                                          <div>
                                              <span className="block font-bold text-xs">Harorat</span>
                                              {boilerIoTDevice && (
                                                  <span className={`text-[8px] ${tempLabelColor}`}>{boilerIoTDevice.deviceId}</span>
                                              )}
                                          </div>
                                      </div>
                                      <div className="flex items-center gap-3">
                                          <div className="flex items-center gap-2">
                                              {tDirection === 'HIGH' && <div className={`p-1 rounded-full ${tStatus === 'CRITICAL' ? 'bg-white/20 text-white' : 'bg-red-100 text-red-600'}`}><ArrowUp size={14} strokeWidth={3} /></div>}
                                              {tDirection === 'LOW' && <div className={`p-1 rounded-full ${tStatus === 'CRITICAL' ? 'bg-white/20 text-white' : 'bg-blue-100 text-blue-600'}`}><ArrowDown size={14} strokeWidth={3} /></div>}
                                              <div className={`px-3 py-1 rounded-[10px] text-sm font-bold flex items-center gap-1 ${tempValueClass}`}>
                                          ...
                                          <tr key={room.id} className={`border-b border-slate-100 hover:bg-slate-50 transition-colors ${roomIdx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}>
                                            ...
                                            <td className="px-3 py-2.5 text-center">
                                              {roomDisplayTemperature !== undefined ? (() => {
                                                const rtStatus = getTemperatureStatusColor(roomDisplayTemperature, config.tempMin, config.tempMax);
                                                let rtClass = "font-bold text-slate-800";
                                                if (rtStatus === 'CRITICAL') rtClass = "font-bold text-red-600";
                                                else if (rtStatus === 'WARNING') rtClass = "font-bold text-amber-600";
                                                return <span className={rtClass}>{roomDisplayTemperature}°C</span>;
                                              })() : (
