...
export const DB = {
    ...
    async deleteBin(id: string): Promise<void> {
        try {
            const token = localStorage.getItem('authToken');
            if (!token) {
                // If no token, just remove from localStorage
                const bins = JSON.parse(localStorage.getItem('smartcity_bins') || '[]');
                const filtered = bins.filter((b: WasteBin) => b.id !== id);
                localStorage.setItem('smartcity_bins', JSON.stringify(filtered));
                return;
            }
            
            await ApiService.deleteWasteBin(id);
            ...
            console.error('API Delete Bin Error:', e);
            throw e;
        }
    },
