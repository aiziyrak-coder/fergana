...
export type Tab = 'DASHBOARD' | 'ANALYTICS' | 'MOISTURE' | 'CLIMATE' | 'WASTE' | 'ECO_CONTROL' | 'AIR' | 'SECURITY' | 'CONSTRUCTION' | 'LIGHT_INSPECTOR' | 'TRANSPORT' | 'CALL_CENTER' | 
  ...
  'CLIMATE_REALTIME' | 'CLIMATE_SCHEDULER' | 'CLIMATE_AUTO' | 'CLIMATE_ENERGY' | 'CLIMATE_REPORTS' | 'ALERTS' | 'LIVESTOCK';
...
export type AnimalType = 'CATTLE' | 'SHEEP' | 'GOAT' | 'HORSE' | 'CAMEL';
export type FarmType = 'CATTLE' | 'SHEEP' | 'GOAT' | 'HORSE' | 'CAMEL' | 'MIXED';
export type HealthStatus = 'HEALTHY' | 'SICK' | 'QUARANTINE' | 'VACCINATED' | 'TREATMENT';
export type ChipStatus = 'ACTIVE' | 'INACTIVE' | 'DAMAGED' | 'LOST';
...
export interface Farm {
  ...
  livestockStats?: Record<string, { count: number; label: string; healthy: number; sick: number }>;
  organizationId?: string;
}
...
export interface LivestockPassport {
  ...
  vaccinationHistory: { date: string; vaccine: string; vet: string }[];
  diseaseHistory: { date: string; disease: string; treatment: string }[];
  weightHistory: { date: string; weight: number }[];
  transferHistory: { date: string; fromOwner: string; toOwner: string }[];
  additionalInfo: Record<string, any>;
}

export interface MicrochipDevice {
  id: string;
  chipId: string;
  livestockId?: string;
  manufacturer: string;
...
export interface LivestockStatistics {
  totalFarms: number;
  totalLivestock: number;
  byType: Record<string, { count: number; label: string }>;
  byHealth: Record<string, { count: number; label: string }>;
  byGender: { MALE: number; FEMALE: number };
  totalMicrochips: number;
  farmsSummary: { id: string; name: string; owner: string; total: number; healthy: number; sick: number }[];
}
