...
class Livestock(models.Model):
    ...
    microchip_id = models.CharField(max_length=50, unique=True, help_text="Microchip identifikator raqami")
    animal_type = models.CharField(max_length=20, choices=ANIMAL_TYPES)
    breed = models.CharField(max_length=100, help_text="Zoti")
    name = models.CharField(max_length=100, blank=True, help_text="Hayvon nomi (ixtiyoriy)")
    gender = models.CharField(max_length=10, choices=GENDER_CHOICES)
    birth_date = models.DateField(null=True, blank=True)
    weight_kg = models.FloatField(default=0, help_text="Vazni (kg)")
    color = models.CharField(max_length=50, blank=True, help_text="Rangi")
    ...
    last_vet_check = models.DateField(null=True, blank=True)
    ...
    def __str__(self):
        return f"{self.microchip_id} - {self.get_animal_type_display()} ({self.breed})"
    
    class Meta:
        ordering = ['-created_at']


class LivestockPassport(models.Model):
    """Chorva pasporti - to'liq ma'lumotlar"""
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    livestock = models.OneToOneField(Livestock, on_delete=models.CASCADE, related_name='passport')
    passport_number = models.CharField(max_length=50, unique=True)
    issue_date = models.DateField()
    issuing_authority = models.CharField(max_length=255, help_text="Bergan tashkilot")
    origin_region = models.CharField(max_length=100, blank=True)
    origin_district = models.CharField(max_length=100, blank=True)
    mother_microchip_id = models.CharField(max_length=50, blank=True, help_text="Onasi microchip ID")
    father_microchip_id = models.CharField(max_length=50, blank=True, help_text="Otasi microchip ID")
    ...
    def __str__(self):
        return f"Pasport #{self.passport_number} - {self.livestock.microchip_id}"
    
    class Meta:
        ordering = ['-created_at']


class MicrochipDevice(models.Model):
    """Microchip qurilmasi"""
    CHIP_STATUS = [
        ('ACTIVE', 'Faol'),
        ('INACTIVE', 'Nofaol'),
    ...
    livestock = models.OneToOneField(Livestock, on_delete=models.CASCADE, related_name='microchip', null=True, blank=True)
    manufacturer = models.CharField(max_length=100, blank=True)
    implant_date = models.DateField(null=True, blank=True)
    implant_location = models.CharField(max_length=100, blank=True, help_text="Qo'yilgan joy (masalan: chap quloq)")
    status = models.CharField(max_length=20, choices=CHIP_STATUS, default='ACTIVE')
    ...
    def __str__(self):
        return f"Chip {self.chip_id} - {self.get_status_display()}"
