...
from .models import (
    ...
    Farm, Livestock, LivestockPassport, MicrochipDevice
)
from .serializers import (
    ...
    FarmSerializer, LivestockSerializer, LivestockPassportSerializer, MicrochipDeviceSerializer
)
import json
import uuid
import requests
            ...
            if org.password == password:  # In production, use Django's password hashing
                ...
                return Response({
                    ...
                    'user': {
                        ...
                        'name': org.name,
                        'role': 'ORGANIZATION',
                        'enabled_modules': ['DASHBOARD', 'WASTE', 'CLIMATE', 'LIVESTOCK']
                    }
                })
...
class FarmListCreateView(APIView):
    def get(self, request):
        ...
        if org_id:
            farms = farms.filter(organization_id=org_id)
...
class LivestockListCreateView(APIView):
    def get(self, request):
        livestock = Livestock.objects.select_related('farm', 'passport', 'microchip').all()
        farm_id = request.query_params.get('farm_id')
        animal_type = request.query_params.get('animal_type')
        health_status_param = request.query_params.get('health_status')
        search = request.query_params.get('search')
        if farm_id:
            livestock = livestock.filter(farm_id=farm_id)
        if animal_type:
            livestock = livestock.filter(animal_type=animal_type)
        if health_status_param:
            livestock = livestock.filter(health_status=health_status_param)
        if search:
            from django.db.models import Q
            livestock = livestock.filter(
                Q(microchip_id__icontains=search) |
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response(LivestockSerializer(livestock).data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)


class LivestockDetailView(APIView):
    def get(self, request, pk):
        livestock = get_object_or_404(Livestock.objects.select_related('farm', 'passport', 'microchip'), pk=pk)
        serializer = LivestockSerializer(livestock)
        return Response(serializer.data)
    
    def put(self, request, pk):
        livestock = get_object_or_404(Livestock, pk=pk)
        serializer = LivestockSerializer(livestock, data=request.data, partial=True)
        if serializer.is_valid():
            livestock = serializer.save()
...
class LivestockPassportListCreateView(APIView):
    def get(self, request):
        passports = LivestockPassport.objects.select_related('livestock').all()
        serializer = LivestockPassportSerializer(passports, many=True)
        return Response(serializer.data)
    
    def post(self, request):
        serializer = LivestockPassportSerializer(data=request.data)
        if serializer.is_valid():
            passport = serializer.save()
            return Response(LivestockPassportSerializer(passport).data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)


class LivestockPassportDetailView(APIView):
    def get(self, request, pk):
        passport = get_object_or_404(LivestockPassport, pk=pk)
        serializer = LivestockPassportSerializer(passport)
        return Response(serializer.data)
    
    def put(self, request, pk):
        passport = get_object_or_404(LivestockPassport, pk=pk)
        serializer = LivestockPassportSerializer(passport, data=request.data, partial=True)
        if serializer.is_valid():
            passport = serializer.save()
            return Response(LivestockPassportSerializer(passport).data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)


class MicrochipDeviceListCreateView(APIView):
    def get(self, request):
        chips = MicrochipDevice.objects.select_related('livestock').all()
        serializer = MicrochipDeviceSerializer(chips, many=True)
        return Response(serializer.data)
    
    def post(self, request):
        serializer = MicrochipDeviceSerializer(data=request.data)
        if serializer.is_valid():
            chip = serializer.save()
            return Response(MicrochipDeviceSerializer(chip).data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)


class MicrochipDeviceDetailView(APIView):
    def get(self, request, pk):
        chip = get_object_or_404(MicrochipDevice, pk=pk)
        serializer = MicrochipDeviceSerializer(chip)
        return Response(serializer.data)
    
    def put(self, request, pk):
        chip = get_object_or_404(MicrochipDevice, pk=pk)
        serializer = MicrochipDeviceSerializer(chip, data=request.data, partial=True)
        if serializer.is_valid():
            chip = serializer.save()
            return Response(MicrochipDeviceSerializer(chip).data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)


@api_view(['GET'])
def lookup_livestock_by_microchip(request, microchip_id):
    """Microchip ID bo'yicha chorva ma'lumotlarini qidirish (scanner uchun)"""
    try:
        livestock = Livestock.objects.select_related('farm', 'passport', 'microchip').get(microchip_id=microchip_id)
        data = LivestockSerializer(livestock).data
        if hasattr(livestock, 'passport'):
            data['passport'] = LivestockPassportSerializer(livestock.passport).data
        if hasattr(livestock, 'microchip'):
            data['microchip_device'] = MicrochipDeviceSerializer(livestock.microchip).data
        return Response(data)
    except Livestock.DoesNotExist:
        return Response({'error': 'Chorva topilmadi', 'microchip_id': microchip_id}, status=status.HTTP_404_NOT_FOUND)


@api_view(['GET'])
def get_livestock_statistics(request):
    """Chorva statistikasi"""
    org_id = request.query_params.get('organization_id')
    farms = Farm.objects.all()
    if org_id:
    ...
    livestock = Livestock.objects.filter(is_active=True)
    if org_id:
        livestock = livestock.filter(farm__organization_id=org_id)
    
    stats = {
        'total_farms': farms.filter(is_active=True).count(),
        'total_livestock': livestock.count(),
        'by_type': {},
        'by_health': {},
        'by_gender': {
            'MALE': livestock.filter(gender='MALE').count(),
            'FEMALE': livestock.filter(gender='FEMALE').count(),
        },
        'total_microchips': MicrochipDevice.objects.filter(status='ACTIVE').count(),
        'farms_summary': []
    }
    
    for animal_type, label in Livestock.ANIMAL_TYPES:
        count = livestock.filter(animal_type=animal_type).count()
        stats['by_type'][animal_type] = {'count': count, 'label': label}
    
    for hs, label in Livestock.HEALTH_STATUS:
        count = livestock.filter(health_status=hs).count()
        stats['by_health'][hs] = {'count': count, 'label': label}
    
    for farm in farms.filter(is_active=True)[:20]:
        farm_livestock = livestock.filter(farm=farm)
        stats['farms_summary'].append({
            'id': str(farm.id),
            'name': farm.name,
            'owner': farm.owner_name,
            'total': farm_livestock.count(),
            'healthy': farm_livestock.filter(health_status='HEALTHY').count(),
            'sick': farm_livestock.filter(health_status='SICK').count(),
        })
