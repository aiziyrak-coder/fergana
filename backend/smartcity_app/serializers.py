...
# ==================== CHORVA (LIVESTOCK) MODULE SERIALIZERS ====================

class MicrochipDeviceSerializer(serializers.ModelSerializer):
    class Meta:
        model = MicrochipDevice
        fields = '__all__'


class LivestockPassportSerializer(serializers.ModelSerializer):
    class Meta:
        model = LivestockPassport
        fields = '__all__'


class LivestockSerializer(serializers.ModelSerializer):
    passport = LivestockPassportSerializer(read_only=True)
    microchip = MicrochipDeviceSerializer(read_only=True)
...
class FarmSerializer(serializers.ModelSerializer):
    ...
    def get_livestock_count(self, obj):
        return obj.livestock.filter(is_active=True).count()
    
    def get_livestock_stats(self, obj):
        livestock = obj.livestock.filter(is_active=True)
        stats = {}
        for animal_type, label in Livestock.ANIMAL_TYPES:
            ...
            if count > 0:
                stats[animal_type] = {
                    'count': count,
                    'label': label,
                    'healthy': livestock.filter(animal_type=animal_type, health_status='HEALTHY').count(),
                    'sick': livestock.filter(animal_type=animal_type, health_status='SICK').count(),
                }
        return stats
